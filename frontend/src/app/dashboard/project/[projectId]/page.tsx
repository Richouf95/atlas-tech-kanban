...
async function ProjectPage({
  ...
  const thisRoomBoards = rooms.filter(room => room.metadata.projectId === projectId);
  
  // Sort the boards by createdAt date (oldest first)
  thisRoomBoards.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
  ...
  return (
    <>
      <h1 className="text-2xl font-bold mt-4 ml-4 uppercase">Project : {firstBoard.metadata.boardName}</h1>
      <div className="p-5">
